...
| Import | Dùng cho |
|--------|----------|
| `@nmvuong92/fluxe` | engine: `defineCell`, `makeServer`, `withInput`, resolver, seo… (KHÔNG có driver data — backend là của bạn ở `app/backend/data.ts`) |
...
const manifest = resolve([home], { name: "dev" });   // profile chỉ giải render
