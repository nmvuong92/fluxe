...
export function Todos({ data }: { data: TodosData }) {
  // Contract-aware hooks: list = nguồn sự thật (initial = SSR, không flash).
  const q = api.listTodos.useQuery({ initial: data.todos });
  // useForm: field suy từ input op addTodo; refetch listTodos sau khi thêm.
  ...
  // Realtime: subscription typed (broker SSE) — tab khác đổi → nhận list mới → refetch.
  ...
  return (
    <div className="card">
      ...
      {form.formError || toggle.error ? <p style={{ color: "crimson" }}>{form.formError || toggle.error}</p> : null}
      <ul className="list">
        {todos.map((t) => (
          <li key={t.id} onClick={() => toggle.mutate({ id: t.id })} className={t.done ? "done" : ""}>
            <span>{t.done ? "✓" : "○"}</span> {t.title}
          </li>
        ))}
      </ul>
