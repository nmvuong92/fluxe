...
( cd backends-native/host-go && PORT=$HOST_PORT FLUXE_UPSTREAM=http://127.0.0.1:$NODE_PORT FLUXE_MANIFEST=../../.fluxe/resolution.json go run . ) >/tmp/fluxe-host.log 2>&1 & pids+=($!)
...
echo "[/todos] — SSR React do Node render, qua Go host:"
echo "  có <ul> SSR? $(curl -s $H/todos | grep -oc '<ul')   (header: $(curl -s -D - $H/todos -o /dev/null | grep -i 'x-fluxe-host' | tr -d '\r'))"
