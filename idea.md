...
> Bốn tenet này + **RCA** + **escape-hatch 3 tầng** = 6 luật nền. Mọi trục bên dưới phải
...
### Tân tiến hơn Filament ở đâu
- **Type-safe đầu-cuối**: column/field/filter/action suy từ contract (Filament dùng mảng PHP — fluxe có kiểu).
- **Realtime mặc định** (4g): bảng cập nhật live, **presence** (ai đang sửa record), khóa lạc quan.
- **Polyglot**: admin nói chuyện qua biên `Backend` → chạy bất kể backend Go/Rust/TS.
- **RCA-native**: mỗi trang admin **là một cell** (resolved như mọi cell) → trang custom = chỉ là cell.
...
- **SEO / head / meta ⭐⭐**: title/OG/meta per route, structured data, sitemap/robots tự
  sinh. Cần cho use-case site/marketing/docs (cạnh tranh Astro). Đang thiếu hẳn.
...
- **i18n/l10n + a11y ⭐**: đã ghi hoãn; giữ trong tầm ngắm, đừng để thành nợ kiến trúc.
