...
- **Erasable-only TS** (enforce qua `erasableSyntaxOnly: true` — root + `fx init` template): **KHÔNG** dùng `enum`, `namespace`, parameter-property (`constructor(private x)`) — chúng là *runtime syntax*, vỡ khi Node type-stripping / tsgo strip type. Thay `enum` bằng `as const` union (`const X = ["a","b"] as const; type X = typeof X[number]`). Ưu tiên `satisfies`/`const` type param ở builder khi cần **giữ literal shape** (không widen) — chỉ khi có nhu cầu đo được, không cargo-cult.
...
- **Cấu trúc app: `app/backend` + `app/frontend` (feature-module).** backend module = folder `app/backend/modules/<x>/`: starter **FLAT** = `<x>.module.ts` (entry `defineModule`) + `contract.ts` (op schema). **Đừng dựng sẵn api/domain/application/infrastructure** — thêm tầng khi module đó ĐO ĐƯỢC phức tạp (promotion path: `guides/data-layer.md`). Repository seam = `backend/db.ts` (interface `TodoStore` + driver memory/drizzle/pg, đổi driver 0 sửa module). Domain rules = pure functions (không factory). Data layer default = **Drizzle** (`--driver=drizzle`; MikroORM/TypeORM loại vì decorator vỡ erasable-only). `defineModule<{db:TodoStore}>({ name, contract, use, resolvers })` **đẩy wiring vào core, KHÔNG make/thread**: `resolvers` = **object KHAI BÁO** `{ op: (input, ctx) => … }` với `ctx.db` tiêm qua `use:{db:"backend"}` (core auto-provide `"backend"` từ `opts.backend`; `needs` tự suy từ `use`). ctx = ResolverCtx (session/publish/span) + injected. domain/ = pure functions (không factory). (Vẫn hỗ trợ `resolvers: (app)=>…` factory nếu cần.) **ĐỪNG** dùng lại pattern `makeXResolvers(store)`. Gom qua `createApp({ plugins:[mod] })`. Scale = promotion path (data-layer.md), không dựng tầng sẵn. frontend theo feature: `app/frontend/features/<feature>/<name>.cell.tsx + <name>.view.tsx`. Hạ tầng: `app/backend/{db,env,contract,server}.ts`, `app/frontend/{layouts,components,i18n,api,profiles}`, `app/backend/tests/` (unit/e2e/helpers/fixtures). Alias `@backend/*`, `@frontend/*`.
...
## Package & publish
- Engine publish dưới tên **`@nmvuong92/fluxe`** (subpath `/react`, `/client`, `/express`, `/fastify`, `/auth`, `/openapi`, `/graphql`).
