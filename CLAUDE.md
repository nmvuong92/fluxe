...
- **Contract DSL (builder, 0 codegen — tRPC-style): `app/contract.ts` dùng `f` (lớp mỏng trên Zod): `f.object/query/mutation/contract`. Types suy TỨC THÌ qua `Infer<>`/`Resolvers<typeof contract>`/`Client<C>` — KHÔNG `fx gen`, KHÔNG `.fluxe/gen`. Client = `createClient<typeof contract>()` (Proxy typed, `@nmvuong92/fluxe/client`, contract import type-only → 0 schema xuống browser). Runtime `/__rpc/<op>` đọc contract trực tiếp: validate Zod (`op.input`) + CSRF (`op.kind==="mutation"`). Resolvers tiêm `{ contract, resolvers }` (fallback `{ backend }`); DB ẩn sau resolver — `actions/rpc` cũ giữ nguyên (lớp THÊM). ĐỪNG quay lại string DSL / codegen.**
