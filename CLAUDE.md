...
- **Cấu trúc app: `app/backend` + `app/frontend` (feature-module).** backend module = folder `app/backend/modules/<x>/`: **entry `<x>.module.ts`** (`defineModule` — mở file này hiểu cả module) + **`api/`** (biên ngoài: `contract.ts`, `resolver.ts`) + **`domain/`** (nghiệp vụ: `service.ts`, `data.ts` tùy chọn). Đọc api→domain = pipeline request. `defineModule({ name, contract, needs, resolvers })` **đẩy wiring vào core**: `resolvers` là factory `(app)=>...` nhận capability qua DI (core auto-provide `"backend"` từ `opts.backend` → `needs:["backend"]` tự nhận, KHÔNG thread store tay). Gom qua `createApp({ plugins:[mod] })`. Scale = tách `api/resolver/` per-op. frontend theo feature: `app/frontend/features/<feature>/<name>.cell.tsx + <name>.view.tsx`. Hạ tầng: `app/backend/{db,env,contract,server}.ts`, `app/frontend/{layouts,components,i18n,api,profiles}`, `app/backend/tests/` (unit/e2e/helpers/fixtures). Alias `@backend/*`, `@frontend/*`.
