...
- **fluxe = CẦU NỐI RCA, KHÔNG tự làm thứ framework đã làm.** ĐÃ GỠ khỏi core: auth/session/scrypt/CSRF/RBAC, rate-limit, storage(upload), jobs(queue), DI container. Những thứ này là việc của **HOST framework** (Express/Hono/Nest) + ecosystem (passport, express-rate-limit, multer, bullmq…) — mount middleware host TRƯỚC fluxe. `/__rpc`·`/__action` KHÔNG kiểm CSRF/rate-limit (host lo); `ctx.session` = host gắn `req.session` (fluxe không verify). **Đừng thêm lại** các thứ đã gỡ vào core. GIỮ trong core: cells/SSR, resolver, contract+/__rpc+validate, realtime (broker/SSE), observability (/_fluxe), seo, i18n, render-cache.
...
- **Contract DSL (builder, 0 codegen — tRPC-style): `app/contract.ts` dùng `f` (lớp mỏng trên Zod): `f.object/query/mutation/contract`. Types suy TỨC THÌ qua `Infer<>`/`Resolvers<typeof contract>`/`Client<C>` — KHÔNG `fx gen`, KHÔNG `.fluxe/gen`. Client = `createClient<typeof contract>()` (Proxy typed, `@nmvuong92/fluxe/client`, contract import type-only → 0 schema xuống browser). Runtime `/__rpc/<op>` đọc contract trực tiếp: validate Zod (`op.input`); CSRF do HOST lo. Resolvers tiêm `{ contract, resolvers }` (fallback `{ backend }`); DB ẩn sau resolver — `actions/rpc` cũ giữ nguyên (lớp THÊM). ĐỪNG quay lại string DSL / codegen.**
- **Auth = INTEGRATION (`@nmvuong92/fluxe/auth`), KHÔNG reinvent engine auth.** Provider (better-auth/lucia/passport) lo OAuth/password/2FA/issue+verify session. fluxe chỉ cho **lớp tích hợp RCA-native**: `bridgeSession(getSession)` (node middleware → gắn `req.session`, mount TRƯỚC fluxe), `protect(role?)` (guard route host), endpoint `/__session` (trả `req.session` JSON), `useSession()` (react `@nmvuong92/fluxe/react`, đọc /__session typed), guard **khai báo**: cell `requireRole`/`requireAuth` + contract op `{ auth: true|role }` (kiểm trong `handleRpc` → 401/403). Session **typed** xuyên cell qua `createCells<B, S>()`. **Đừng** build OAuth/password/cookie engine vào core.
- **Typed routes:** `route` literal → `ctx.input` suy qua `RouteParams<R>` (template-literal type). `createCells<B, S = Session>()` (`app/cell.ts`) bind backend + session type **một lần**, vẫn suy route+O; `defineCell` từ package = B/S mặc định. Cell registry `app/app.ts` type `CellDef<any,any,any,any>` (4 generic I,O,B,S — `actions` session-agnostic `Action<any,any,B,any>` để khớp `withInput`).
...
`@nmvuong92/fluxe` / `/react` / `/client` / `/express` / `/hono` / `/nest` / `/auth` (KHÔNG `../../../src/...` hay
...
## Package & publish
- Engine publish dưới tên **`@nmvuong92/fluxe`** (subpath `/react`, `/client`, `/express`, `/hono`, `/nest`, `/auth`).
